...
def on_mouse_press(x, y, button, modifiers):
    if button == 1:  # left
        world.targett.pos = Vector2D(x, y)
...
def on_key_press(symbol, modifiers):
    if symbol == KEY.P:
        ...
        world.agent.mode = AGENT_MODES[symbol]
    ## LAB 05 TASK 2: Add agent by pressing a key
    elif symbol == KEY.A:
        world.add_enemy('seek', True, world.enemytarget, 'RED')
...
if __name__ == '__main__':
    ...
    win = window.Window(width=1500, height=1000, vsync=True, resizable=True)
    glEnable(GL_BLEND)
